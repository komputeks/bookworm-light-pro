    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-2 text-2xl font-bold text-text-dark dark:text-white">Create account</h1>
          <p className="text-text">Join the community of writers</p>
        </div>

        <form onSubmit={handleSubmit} className="card space-y-4 p-6">
          <div>
            <label className="mb-1 block text-sm font-semibold text-text-dark dark:text-white">Username</label>
            <div className="relative">
              <FaUser className="absolute left-3 top-1/2 -translate-y-1/2 text-text-light" />
              <input type="text" required value={username} onChange={(e) => setUsername(e.target.value)}
                className="input pl-10" placeholder="johndoe" pattern="[a-zA-Z0-9_-]+" minLength={3} />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)}
                className="input pl-10" placeholder="At least 6 characters" minLength={6} />
            </div>
          </div>
